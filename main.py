...
def update_mahasiswa_kelulusan(nrp: str, data: KelulusanRequest):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("""
...
# =====================================================
# PREDIKSI & ADVANCED XAI KELULUSAN (KWS)
# =====================================================
...
def create_semester(data: SemesterRequest):
    ...
    try:
        ...
        conn.commit()
        return {"message": "Data semester berhasil ditambahkan"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        conn.close()
...
def update_semester(nrp: str, data: SemesterRequest):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("""
    ...
    conn.commit()
    conn.close()
    return {"message": "Data semester berhasil diupdate"}
...
def delete_semester(nrp: str):
    ...
    conn.commit()
    conn.close()
    return {"message": "Data semester berhasil dihapus"}

# =====================================================
# PREDIKSI & ADVANCED XAI SEMESTER (EWS)
# =====================================================
